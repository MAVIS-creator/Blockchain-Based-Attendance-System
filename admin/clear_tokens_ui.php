...
document.getElementById('searchClearBtn').addEventListener('click', function(){
  ...
  Swal.fire({ title:'Confirm clear', text:'Clear any entries matching this key?', icon:'warning', showCancelButton:true }).then(function(resp){ if (!resp.isConfirmed) return; var p = new URLSearchParams(); p.append('matric', v); p.append('fingerprint', v); if (window.ADMIN_CSRF_TOKEN) p.append('csrf_token', window.ADMIN_CSRF_TOKEN); fetch('clear_device.php', { method:'POST', body: p }).then(r=>r.json()).then(j=>{ Swal.fire('Cleared','Result: '+JSON.stringify(j),'success'); }).catch(e=>Swal.fire('Error','Could not clear','error')); });
});

// per-token actions
Array.from(document.getElementsByClassName('clear-token')).forEach(function(b){ b.addEventListener('click', function(){ var t=this.getAttribute('data-token'); if (!t) return; Swal.fire({ title:'Clear & Revoke', text:'Clear server-side entries for this token AND revoke it on clients?', icon:'warning', showCancelButton:true }).then(function(res){ if (!res.isConfirmed) return; var p1 = new URLSearchParams(); p1.append('token', t); p1.append('days', 7); if (window.ADMIN_CSRF_TOKEN) p1.append('csrf_token', window.ADMIN_CSRF_TOKEN); fetch('revoke_entry.php', { method:'POST', body: p1 }).then(function(){ var p2 = new URLSearchParams(); p2.append('fingerprint', t); if (window.ADMIN_CSRF_TOKEN) p2.append('csrf_token', window.ADMIN_CSRF_TOKEN); return fetch('clear_device.php', { method:'POST', body: p2 }); }).then(r=>r.json()).then(j=>{ Swal.fire('Done','Cleared & Revoked','success').then(()=>location.reload()); }).catch(e=>Swal.fire('Error','Operation failed','error')); }); }); });

Array.from(document.getElementsByClassName('revoke-token')).forEach(function(b){ b.addEventListener('click', function(){ var t=this.getAttribute('data-token'); if (!t) return; Swal.fire({ title:'Revoke token', input:'number', inputLabel:'Expiry (days)', inputValue:7, showCancelButton:true }).then(function(res){ if (!res.value) return; var days = parseInt(res.value||7,10); var p = new URLSearchParams(); p.append('token', t); p.append('days', days); if (window.ADMIN_CSRF_TOKEN) p.append('csrf_token', window.ADMIN_CSRF_TOKEN); fetch('revoke_entry.php', { method:'POST', body: p }).then(r=>r.json()).then(j=>{ Swal.fire('Revoked','Result saved','success'); }).catch(e=>Swal.fire('Error','Could not revoke','error')); }); }); });

document.getElementById('clearAllTokens')?.addEventListener('click', function(){ var tokens = Array.from(document.querySelectorAll('button.clear-token')).map(b=>b.getAttribute('data-token')); if (!tokens.length) return Swal.fire('Empty','No tokens','info'); Swal.fire({ title:'Clear & Revoke All', text:'Clear server-side entries for ALL tokens found in logs and revoke them?', icon:'warning', showCancelButton:true }).then(function(resp){ if (!resp.isConfirmed) return; var promises = tokens.map(function(t){ var p1 = new URLSearchParams(); p1.append('token', t); p1.append('days', 7); if (window.ADMIN_CSRF_TOKEN) p1.append('csrf_token', window.ADMIN_CSRF_TOKEN); return fetch('revoke_entry.php', { method:'POST', body: p1 }).then(function(){ var p2 = new URLSearchParams(); p2.append('fingerprint', t); if (window.ADMIN_CSRF_TOKEN) p2.append('csrf_token', window.ADMIN_CSRF_TOKEN); return fetch('clear_device.php', { method:'POST', body: p2 }); }).then(r=>r.json()); }); Promise.all(promises).then(results=>{ Swal.fire('Done','Cleared & Revoked all tokens','success').then(()=>location.reload()); }).catch(e=>Swal.fire('Error','Error clearing some tokens','error')); }); });

document.getElementById('revokeAllTokens')?.addEventListener('click', function(){ var tokens = Array.from(document.querySelectorAll('button.revoke-token')).map(b=>b.getAttribute('data-token')); if (!tokens.length) return Swal.fire('Empty','No tokens','info'); Swal.fire({ title:'Revoke All Tokens', input:'number', inputLabel:'Expiry (days)', inputValue:7, showCancelButton:true }).then(function(res){ if (!res.value) return; var days = parseInt(res.value||7,10); var promises = tokens.map(function(t){ var p = new URLSearchParams(); p.append('token', t); p.append('days', days); if (window.ADMIN_CSRF_TOKEN) p.append('csrf_token', window.ADMIN_CSRF_TOKEN); return fetch('revoke_entry.php', { method:'POST', body: p }).then(r=>r.json()); }); Promise.all(promises).then(results=>{ Swal.fire('Done','Revoked all tokens','success'); }).catch(e=>Swal.fire('Error','Error revoking some tokens','error')); }); });
